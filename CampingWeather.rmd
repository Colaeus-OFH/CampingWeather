...
#)
        qryStr = "https://data.novascotia.ca/resource/kafq-j9u4.json?$select=site_id,datetimeutc,air_temperature,max_wind_gust_speed, avg_wind_speed,precipitation_1_hour,date_extract_m(datetimeutc) as month,date_extract_d(datetimeutc) as theday&$where=month = 1 AND theday = 12 AND site_id= 'RNSAV'"
        df <- read.socrata(
...
hist(as.numeric(df$precipitation_1_hour))
